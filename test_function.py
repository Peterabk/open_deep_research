import asyncio
import datetime
from src.open_deep_research.utils import tavily_search_async, firecrawl_search_async
...
async def test_firecrawl_search():
    """Test function for tavily_search_async"""
    print(f"[{datetime.datetime.now()}] Starting firecrawl search test...")
    ...
    try:
        results = await firecrawl_search_async(
            search_queries=search_queries,
            max_results=3,
            include_raw_content=True,
            topic="general"
...
if __name__ == "__main__":
    ...
    asyncio.run(test_firecrawl_search())
